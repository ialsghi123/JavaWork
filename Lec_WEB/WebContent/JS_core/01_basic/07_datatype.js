...
//true / false
{
    ...
    //객체로 생성 가능
    //가능은 하지만 비추
    const a = new Boolean(false); //false 값을 갖는 boolen 객체
    ...
    //new 를 사용하지않고 boolean 만들기
    const b= Boolean(false); //boolean() 함수
    console.log('b=',b,typeof b);

    if(b) { //<-new를 사용하여 생성하면 조건식에서 '참' 으로 판정된다
    console.log(b,'참입니다');
...
console.log('\n<number>')
{
    ...
    console.log('c=',c,typeof c); //number
    
    let d = Number(123); //number로 형변환 됨!
    console.log('c=',c,typeof c); //number
...
console.log('\n<symbol>');
{
    ...
    const c = Symbol('Mark');
    const d = Symbol('Mark'); //c,d는 같은 것이 아니다
